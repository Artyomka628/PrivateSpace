...
def run_win_script():
    global win_script_started
    if not win_script_started:
        bat_file_path = os.path.join(os.getcwd(), 'C:\\Program Files\\PrivateSpace\\win.bat')
        subprocess.Popen(bat_file_path, creationflags=subprocess.CREATE_NO_WINDOW)
        win_script_started = True

root = tk.Tk()
root.title("Загрузка Windows")
...
icon_path = r"C:\Program Files\PrivateSpace\Icons\win.ico"
...
label2 = tk.Label(root, text="Загрузка Windows...", font=("Helvetica", 14), bg="#343deb", fg="white")
label2.pack(pady=20)

win_script_started = False
run_win_script()
