...
def update_progress():
    ...
    if next_value == 100:
        label2.config(text="Похоже, Windows не удалось выключить компьютер.\nВам придётся сделать это самостоятельно.")
    else:
...
def run_shutdown_script():
    ...
    if not shutdown_started:
        bat_file_path = os.path.join(os.getcwd(), 'C:\\Program Files\\PrivateSpace\\Shutdown.bat')
        subprocess.Popen(bat_file_path, creationflags=subprocess.CREATE_NO_WINDOW)
        shutdown_started = True

root = tk.Tk()
root.title("Завершение работы")
...
icon_path = r"C:\Program Files\PrivateSpace\Icons\shutdown.ico"
...
label2 = tk.Label(root, text="Завершение работы...", font=("Helvetica", 14), bg="#343deb", fg="white")
label2.pack(pady=20)
